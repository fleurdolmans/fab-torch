...
from boltzmann_generators_3d.fab.utils.logging import Logger, ListLogger
...
class BufferTrainer:
    ...
    def run(
        ...
        for i in pbar:
            ...
            x_ais, log_w_ais = self.model.annealed_importance_sampler.sample_and_log_weights(batch_size)
            x_ais = x_ais.detach()
            ...
            info.update(loss=loss.cpu().detach().item(), step=i, grad_norm=grad_norm.cpu().detach().item())
