...
class ManyWellEnergy(DoubleWellEnergy, TargetDistribution):
    ...
    def __init__(self, dim=4, use_gpu: bool = True, normalised: bool = False, a=-0.5, b=-6.0, c=1.0):
        assert dim % 2 == 0
        self.n_wells = dim // 2
    ...
    def get_modes_test_set_iterator(self, batch_size: int):
        ...
        if self.dim < self.max_dim_for_all_modes:
            ...
            test_set[:, torch.arange(self.dim) % 2 == 0] = -self.centre + self.centre * 2 * torch.randint(
    ...
    def log_prob(self, x):
        log_prob = torch.sum(
            torch.stack([super(ManyWellEnergy, self).log_prob(x[:, i * 2 : i * 2 + 2]) for i in range(self.n_wells)]),
            dim=0,
        )
        if self.normalised:
...
if __name__ == "__main__":
    ...
    for i in range(n_rows):
        ...
        plot_marginal_pair(samples_modes, ax=axs[i, 1], bounds=plotting_bounds, marginal_dims=(i * 2, i * 2 + 1))
