...
class MetropolisSampler:
    ...
    def run(self, x, nsteps, diff=False):
        ...
        # single-particle moves for multi-particle configs (x.ndim == 2),
        # global moves for single-point potentials (x.ndim == 1)
        multi_particle = x.ndim == 2

        for i in tqdm(range(nsteps)):
            E_current = self.model.get_energy(x)

            if multi_particle:
                # Pick one random solvent particle (index 1+) and displace it
                idx = np.random.randint(1, len(x))
                delta = self.sigma * np.random.randn(x.shape[1])
                x_proposed = x.copy()
                x_proposed[idx] += delta
            else:
                delta = self.sigma * np.random.randn(*x.shape)
                x_proposed = x + delta
        ...
        self.acceptance_rate = n_accept / nsteps
