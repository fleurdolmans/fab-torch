...
if __name__ == "__main__":
    ...
    double_well_light_marginal_per_dim = (double_well_marginal < -1.0) & (double_well_marginal > -2.1)
    ...
    marginal_condition = (double_well_light_marginal_per_dim[:, 0] == True) & (
        double_well_light_marginal_per_dim[:, 1] == True
    )
    ...
    for i in range(n_rows):
        for j in range(n_rows):
            plot_marginal_pair(
                marginal_samples, ax=axs[i, j], marginal_dims=(i + i_start, j + j_start), bounds=(-3, 3), alpha=0.2
            )
    plt.show()
