...
from generator  import build_solute_spline_flow
...
# --- System ---
N_PARTICLES    = 37          # 1 solute + 36 solvent (needed for energy calc in system)
N_SOLVENT      = 36
DIM            = 2
L_BOX          = 5.0
SIGMA          = 1.1
...
TAIL_BOUND     = 7.0         # spline tail bound (default from build_solute_spline_flow)
...
ENERGY_CUTOFF       = 1000.0   # used to filter out extreme outliers in log_w and log_p when filling the buffer
...
T_SCHEDULE = [
    ...
    (1.0,  80),   # stage 4: true physical target
]

# --- I/O ---
SAVE_DIR        = str(_HERE / "runs" / "fab_spline")
# Set to the path of a pretrained spline flow .pt file, or None to train from scratch.
# Example: str(_HERE.parent / "Notebooks" / "Trained_models" / "Solute" / "my_spline_model")
PRETRAINED_PATH = str(_HERE.parent / "Notebooks" / "Trained_models" /
                      "Solute" / "Center" / "Spline" /"model_KL_v4.3_center")
...
# MD_DATA_PATH = str(_HERE.parent / "Notebooks" / "MC_data" / "Solute2D_prod_L5_center.npz")
MD_DATA_PATH = None
...
def main():
    ...
    target       = build_target()
    flow_adapter = SoluteFlowFAB(flow)
    ...
    for stage_idx, (temp, n_iters) in enumerate(T_SCHEDULE):
        ...
        # Update target temperature; rebuild buffer because stored log_w depend on T.
