...
parser.add_argument(
    "--config", type=str, default="../config/bm.yaml", help="Path config file specifying model " "architecture"
)
parser.add_argument(
    "--config_ais",
    type=str,
    default=None,
    help="Path config file specifying parameters for " "annealed importance sampling",
)
...
parser.add_argument("--seed", type=int, default=0, help="Seed to be used for sampling")
parser.add_argument(
    "--n_samples", type=int, default=1000000, help="Number of samples to be drawn " "from the base model"
)
parser.add_argument(
    "--n_ais_samples",
    type=int,
    default=-1,
    help="Number of samples to be drawn with " "AIS, if -1 same number of samples " "as from base model are drawn",
)
parser.add_argument(
    "--batch_size",
    type=int,
    default=None,
    help="Batch size to be used for sampling, " "if None the one from config is used",
)
...
if n_ais_samples > 0:
    ...
    for i in range(n_batches):
        if i == n_batches - 1:
            ...
            end_ = batch_size
        point, lw = model.annealed_importance_sampler.sample_and_log_weights(batch_size, purpose="draw samples")
        z = point.x
