...
from boltzmann_generators_3d.fab.types_ import LogProbFunc
from boltzmann_generators_3d.fab.sampling_methods.transition_operators.base import TransitionOperator
from boltzmann_generators_3d.fab.types_ import Distribution
from boltzmann_generators_3d.fab.utils.numerical import effective_sample_size
from boltzmann_generators_3d.fab.sampling_methods.base import get_intermediate_log_prob, create_point, Point
...
class AnnealedImportanceSampler:
    ...
    def __init__(
        ...
        self.B_space = self.setup_distribution_spacing(distribution_spacing_type, n_intermediate_distributions)
    ...
    def sample_and_log_weights(
        ...
        log_w = get_intermediate_log_prob(point, self.B_space[1], self.alpha, self.p_target) - log_prob_p0
        point, log_w = self._remove_nan_and_infs(
            point, log_w, descriptor=f"{'' if purpose is None else purpose} chain init"
        )
        ...
        if logging:
            with torch.no_grad():
                ...
                self._logging_info = LoggingInfo(ess_base=ess_base, ess_ais=ess_ais, log_Z=log_Z.cpu().item())
    ...
    def setup_distribution_spacing(
        ...
        if distribution_spacing_type == "geometric":
            ...
            n_intermediate_geomspace_points = n_intermediate_distributions - n_intermediate_linspace_points - 1
    ...
    def generate_eval_data(
        ...
        for i in range(n_batches):
            ...
            point, base_log_w = self._remove_nan_and_infs(point, base_log_w, descriptor="eval chain init")

            # append base samples and log probs
            base_samples.append(point.x.detach().cpu())
            base_log_w_s.append(base_log_w.detach().cpu())

            log_w = get_intermediate_log_prob(point, self.B_space[1], self.alpha, self.p_target) - point.log_q
            # Move through sequence of intermediate distributions via MCMC.
            for j in range(1, self.n_intermediate_distributions + 1):
                point, log_w = self.perform_transition(point, log_w, j)

            point, log_w = self._remove_nan_and_infs(point, log_w, descriptor="eval chain end", raise_exception=False)
    ...
    def _remove_nan_and_infs(
        self, point: Point, log_w: torch.Tensor, descriptor: str = "chain init", raise_exception: bool = True
    ) -> Tuple[Point, torch.Tensor]:
        ...
        num_invalid = torch.sum(~valid_indices)
        if num_valid == 0:  # no valid indices
            if raise_exception:
        ...
        if valid_indices.all():
            ...
            print(
                f"{num_invalid} nan/inf samples/log-probs/log-weights encountered (out of {num_valid + num_invalid}) "
                f"at {descriptor}."
            )
