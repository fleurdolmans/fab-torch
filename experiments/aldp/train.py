...
parser = argparse.ArgumentParser(description="Train Boltzmann Generator " "with various objectives")
...
parser.add_argument("--tlimit", type=float, default=None, help="Number of hours after which to stop training")
parser.add_argument("--mode", type=str, default="gpu", help="Compute mode, can be cpu, or gpu")
parser.add_argument(
    "--precision",
    type=str,
    default="double",
...
if "lr_scheduler" in config["training"]:
    if config["training"]["lr_scheduler"]["type"] == "exponential":
        lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=optimizer, gamma=config["training"]["lr_scheduler"]["rate_decay"]
        )
        lr_step = config["training"]["lr_scheduler"]["decay_iter"]
    elif config["training"]["lr_scheduler"]["type"] == "cosine":
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=optimizer, T_max=config["training"]["max_iter"]
        )
        lr_step = 1
    elif config["training"]["lr_scheduler"]["type"] == "cosine_restart":
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer=optimizer, T_0=config["training"]["lr_scheduler"]["restart_iter"]
        )
        lr_step = 1
else:
    lr_scheduler = None
...
if lr_warmup:
    warmup_scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lambda s: min(1.0, s / config["training"]["warmup_iter"])
    )
...
max_grad_norm = None if not "max_grad_norm" in config["training"] else config["training"]["max_grad_norm"]
...
ndim = 60
loss_type = "fab_alpha_div" if "loss_type" not in config["fab"] else config["fab"]["loss_type"]
    ...
    if latest_cp is not None:
        ...
        for log_label, log_hist in zip(log_labels, log_hists):
            ...
            if os.path.exists(log_path):
                ...
                log_hist[:, :] = log_hist_
                log_hist.resize(np.sum(log_hist_[:, 0] <= start_iter), log_hist_.shape[1], refcheck=False)
    ...
    if rb_config["type"] == "uniform":
        ...
        def initial_sampler():
            point, log_w = model.annealed_importance_sampler.sample_and_log_weights(
                batch_size, logging=False, purpose="init buffer fill"
            )
        ...
        if os.path.exists(buffer_path):
            initial_sampler = lambda: (torch.zeros(batch_size, ndim), torch.zeros(batch_size), torch.ones(batch_size))
        else:
    ...
    if filter_chirality_train:
        if loss_type == "fab_alpha_div":
            ...
            def modified_loss(bs):
                point_ais, log_w_ais = model.annealed_importance_sampler.sample_and_log_weights(bs, purpose="alpha_div")
...
for it in range(start_iter, max_iter):
    ...
    if loss_type == "forward_kl" or lam_fkld is not None:
        ...
        if rb_config["type"] == "uniform":
            if it % rb_config["n_updates"] == 0:
                # Sample
                point_ais, log_w_ais = model.annealed_importance_sampler.sample_and_log_weights(
                    batch_size, logging=False, purpose="buffer fill"
                )
                # Filter chirality
                if filter_chirality_train:
                    ind_L = filter_chirality(point_ais.x)
                ...
                # Sample from buffer
                buffer_sample = buffer.sample_n_batches(batch_size=batch_size, n_batches=rb_config["n_updates"] - 1)
            ...
            if it % rb_config["n_updates"] == 0:
                ...
                # Sample from buffer
                buffer_sample = buffer.sample_n_batches(batch_size=batch_size, n_batches=rb_config["n_updates"])
                buffer_iter = iter(buffer_sample)

            # Get batch from buffer
            x, log_w, log_q_old, indices = next(buffer_iter)
            x, log_w, log_q_old, indices = x.to(device), log_w.to(device), log_q_old.to(device), indices.to(device)
    ...
    if (it + 1) % log_iter == 0 or it == max_iter - 1:
        # Loss
        np.savetxt(os.path.join(log_dir, "loss.csv"), loss_hist, delimiter=",", header="it,loss", comments="")
        # Gradient clipping
        if grad_clipping:
            np.savetxt(
                os.path.join(log_dir, "grad_norm.csv"),
                grad_norm_hist,
                delimiter=",",
                header="it,grad_norm",
                comments="",
        ...
        base_samples, base_log_w, ais_samples, ais_log_w = model.annealed_importance_sampler.generate_eval_data(
        ...
        ess_append = np.array(
            [
                [
                    it + 1,
                    effective_sample_size(base_log_w, normalised=False),
                    effective_sample_size(ais_log_w, normalised=False),
                ]
            ]
        )
        ess_hist = np.concatenate([ess_hist, ess_append])
        np.savetxt(os.path.join(log_dir, "ess.csv"), ess_hist, delimiter=",", header="it,flow,ais", comments="")
        if use_gpu:
    ...
    if (it + 1) % checkpoint_iter == 0 or it == max_iter - 1:
        ...
        if lr_scheduler is not None:
            torch.save(lr_scheduler.state_dict(), os.path.join(cp_dir, "lr_scheduler.pt"))
        if lr_warmup:
        ...
        evaluate_aldp(
            ...
            test_data,
            log_prob_fn,
            model.target_distribution.coordinate_transform,
            it,
            metric_dir=log_dir_flow,
