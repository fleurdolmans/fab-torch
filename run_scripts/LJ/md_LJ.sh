#!/bin/bash
#SBATCH --job-name=md-LJ
#SBATCH --output=logs/slurm-LJ-%j.out
#SBATCH --error=logs/slurm-LJ-%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Create MD data for LJ system with PBC and plot diagnostics

srun python "${MAIN_DIR}/data_generation/md_data_LJ.py" \
  --config-name make_md_data_LJ \
  simulation_version="v16" \
  create_md=true \
  validate_md=true \
  solid=true \
  temperature=40 \
  # solute_sigma_nm=0.32 \
  # solvent_epsilon_kjmol=0.996 \
  # solute_epsilon_kjmol=0.996 \
  # solvent_sigma_nm=0.30 \
