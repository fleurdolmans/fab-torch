...
def make_aldp_model(config, device):
    ...
    # Target distribution
    transform_mode = "mixed" if not "transform" in config["system"] else config["system"]["transform"]
    shift_dih = False if not "shift_dih" in config["system"] else config["system"]["shift_dih"]
    ...
    if transition_type == "hmc":
        ...
        raise NotImplementedError("The transition operator " + config["fab"]["transition_type"] + " is not implemented")
