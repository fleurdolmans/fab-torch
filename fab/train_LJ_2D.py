...
from fab.utils.logging import ListLogger, Logger, WandbLogger
...
class TrainerLJ2D:
    def __init__(
        ...
        dist_ssolv: float = 0.30,
        dist_solute: float = 0.32,
        mixing: float = 0.0,
    ...
    def _sample_flow_cartesian(self, n_samples: int, batch_size: int = 256) -> torch.Tensor:
        ...
        n_atoms = target_dist.n_particles
        return x.view(n_samples, n_atoms, 2)
    ...
    def run(
        ...
        checkpoint_iter = list(np.linspace(0, n_iterations, n_checkpoints + 1, dtype=int)[1:]) if n_checkpoints else []
        eval_iter = list(np.linspace(0, n_iterations, n_eval + 1, dtype=int)[1:]) if n_eval is not None else []
        plot_iter = list(np.linspace(0, n_iterations, n_plot + 1, dtype=int)[1:]) if n_plot is not None else []
