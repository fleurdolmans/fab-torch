...
python ${LOGS_DIR}/${PROJECT_NAME}/experiments/solvation/run.py \\
  ...
  target.energy_cut=1e4 target.energy_max=1e10 \\
  target.boundary_condition=pbc fab.loss_type=flow_alpha_2_div_nis fab.use_ais=false \\
  ...
  training.overlap_penalty=10 training.mixing=0.0 training.energy_mode=full\\
