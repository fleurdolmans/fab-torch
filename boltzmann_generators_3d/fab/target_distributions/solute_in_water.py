...
from boltzmann_generators_3d.fab.utils.logging import Logger
from boltzmann_generators_3d.fab.target_distributions.base import TargetDistribution
from boltzmann_generators_3d.fab.target_distributions.boltzmann import TransformedBoltzmann, TransformedBoltzmannParallel

from boltzmann_generators_3d.fab.transforms import (
    ...
    def __init__(
        ...
        if self.transform_version is None:
            ...
            if self.boundary_condition == "pbc":
                raise ValueError(
                    "GPS transform is not valid for PBC systems: the global solute-frame rotation "
                    "changes MIC distances and creates spurious inter-molecular clashes. "
                    "Use GPR or LGT for PBC systems instead."
                )
