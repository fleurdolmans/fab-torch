...
python ${LOGS_DIR}/${PROJECT_NAME}/experiments/solvation/run.py \\
  ...
  flow.hidden_units=128 flow.base.type=structured-gauss flow.base.learn_mean_var=true flow.type=coupled-spline-nf \\
  flow.layers=12 flow.blocks_per_layer=4 flow.group_size=6 flow.tail_bound=3 \\
  training.checkpoint_load_dir=/home/fdolmans/HDD/results/fab/SoluteInwater/so2_in_water/MD_training/2026-03-26/22-26-22_695798 \\
  training.lr=3e-5 training.wd=1e-6 training.batch_size=256 evaluation.eval_batch_size=64 \\
  training.max_grad_norm=10 training.warmup_iter=75\\
  training.overlap_penalty=0 training.mixing=0.0 training.energy_mode=full target.transform_version=v1\\
  training.n_iterations=250 training.buffer.use=false training.buffer.prioritised=false training.lr_scheduler.decay_iter=250 \\
  evaluation.n_eval=25 evaluation.n_plots=25 evaluation.n_checkpoints=1
EOF
