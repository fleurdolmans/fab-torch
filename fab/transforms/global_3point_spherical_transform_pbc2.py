...
class PBCGlobal3PointSphericalTransform2(nf.flows.Flow):
    ...
    Internal coords i:
      - solute: shape only -> (r1, r2, theta)                  -> 3 dims
      - waters:
          each water: O position in solute body frame          -> 3 dims
                    + rotation vector in solute body frame     -> 3 dims

    Solute body frame:
      e1 = along S->O1
      e3 = normal to O-S-O plane
      e2 = e3 x e1
    ...
    def __init__(self, L: float, system=None, transform_data=None, internal_dim=None):
        ...
        with torch.no_grad():
            ...
            # Reference rigid water geometry from first water
            w0_start = self.n_solute
            ref = x0[:, w0_start:w0_start + 3, :]  # (1,3,3) [O,H,H]
            ref = ref[0]  # (3,3)

            ref_O = ref[0:1, :]
            ref_rel = ref - ref_O  # O at origin

            self.ref_H1 = ref_rel[1].clone()  # (3,)
            self.ref_H2 = ref_rel[2].clone()  # (3,)

            ref_cent = ref_rel.mean(dim=0, keepdim=True)
            self.ref_water_kabsch = (ref_rel - ref_cent).clone()  # (3,3)

        self.internal_dim = internal_dim if internal_dim is not None else (3 + 6 * self.n_waters)
        print("internal_dim in transform:", self.internal_dim)
    ...
    def rotvec_to_rotmat(self, w: torch.Tensor) -> torch.Tensor:
        """
        w: (B,3) rotation vector
        returns R: (B,3,3), column-vector convention
        """
        B = w.shape[0]
    ...
    def rotmat_to_rotvec(self, R: torch.Tensor) -> torch.Tensor:
        """
        R: (B,3,3), column-vector convention
        returns w: (B,3)
        """
        B = R.shape[0]
        trace = R[:, 0, 0] + R[:, 1, 1] + R[:, 2, 2]
    ...
    def solute_body_frame(self, sol_rel: torch.Tensor) -> torch.Tensor:
        """
        Build solute body frame from solute relative coordinates.

        sol_rel: (B,3,3), solute in coordinates relative to atom 0
                 atom 0 = S at origin, atom 1 = O1, atom 2 = O2

        returns Q: (B,3,3) with columns [e1, e2, e3]
          body -> lab : v_lab = Q @ v_body
          lab -> body : v_body = Q^T @ v_lab
        """
        v1 = sol_rel[:, 1, :]   # S -> O1
        v2 = sol_rel[:, 2, :]   # S -> O2
        ...
        Q = torch.stack([e1, e2, e3], dim=-1)  # columns
        return Q

    # ---------- Kabsch ----------
    @staticmethod
    def kabsch_rotation_row(X: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
        """
        Row-vector convention:
          returns R_row such that X @ R_row ≈ Y
    ...
    def inverse(self, x: torch.Tensor):
        ...
        # Make solute whole and define all coordinates relative to solute atom 0
        sol = self.make_whole_solute(x[:, :3, :])   # (B,3,3)
        origin = sol[:, 0:1, :]                     # (B,1,3)
        x_rel = self.mic(x - origin, self.L)        # (B,N,3), lab frame rel to solute atom 0

        # Solute shape coordinates
        sol_rel = x_rel[:, :3, :]                   # S at 0, O1, O2 in lab frame
        v1 = sol_rel[:, 1, :]
        v2 = sol_rel[:, 2, :]

        r1 = torch.linalg.norm(v1, dim=-1, keepdim=True)
        r2 = torch.linalg.norm(v2, dim=-1, keepdim=True)
        theta = self.angle_abc(sol_rel[:, 1, :], sol_rel[:, 0, :], sol_rel[:, 2, :]).unsqueeze(1)

        pieces = [r1, r2, theta]
        logdet = torch.zeros((B,), device=x.device, dtype=x.dtype)

        # Build solute body frame
        Q = self.solute_body_frame(sol_rel)         # body -> lab
        Qt = Q.transpose(1, 2)                      # lab -> body

        # Reference water geometry for Kabsch
        Yref = self.ref_water_kabsch.to(x.device, x.dtype).unsqueeze(0).expand(B, 3, 3)

        for k in range(self.n_waters):
            s = self.n_solute + 3 * k
            w = x_rel[:, s:s + 3, :]                # (B,3,3) [O,H,H], lab frame rel to solute atom 0
            w = self.make_whole_water(w)

            # Oxygen position in lab frame rel to solute origin
            O_lab = w[:, 0, :]                      # (B,3)

            # Convert oxygen position to solute body frame
            O_body = torch.bmm(Qt, O_lab.unsqueeze(-1)).squeeze(-1)

            # Infer water rotation in lab frame from rigid geometry
            w_rel = w - w[:, 0:1, :]               # O at 0
            w_cent = w_rel - w_rel.mean(dim=1, keepdim=True)

            # Kabsch in row-vector convention: Yref @ R_row ≈ w_cent
            R_row = self.kabsch_rotation_row(Yref, w_cent)
            R_lab = R_row.transpose(1, 2)          # convert to column-vector convention

            # Rotate water orientation into solute body frame
            R_body = torch.bmm(Qt, R_lab)
            omega_body = self.rotmat_to_rotvec(R_body)
    ...
    def forward(self, i: torch.Tensor):
        ...
        for k in range(self.n_waters):
            O_body = i[:, idx:idx + 3]             # (B,3)
            omega_body = i[:, idx + 3:idx + 6]     # (B,3)
            idx += 6

            # In canonical frame, body coords == lab coords
            O_abs = center + O_body
            R_body = self.rotvec_to_rotmat(omega_body)

            x[:, self.n_solute + 3 * k + 0, :] = O_abs

            H1 = (R_body @ H1_ref.expand(B, 3, 1)).squeeze(-1)
            H2 = (R_body @ H2_ref.expand(B, 3, 1)).squeeze(-1)
