...
def evaluate_aldp(z_sample, z_test, log_prob, transform, iter, metric_dir=None, plot_dir=None, batch_size=1000):
    """
    ...
    nbins_ram = 64
    eps_ram = 1e-10
    hist_ram_test = np.histogram2d(phi_d, psi_d, nbins_ram, range=[[-np.pi, np.pi], [-np.pi, np.pi]], density=True)[0]
    hist_ram_gen = np.histogram2d(phi, psi, nbins_ram, range=[[-np.pi, np.pi], [-np.pi, np.pi]], density=True)[0]
    kld_ram = (
    ...
    if metric_dir is not None:
        ...
        if os.path.exists(kld_path):
            ...
            kld_hist = kld_append
        np.savetxt(kld_path, kld_hist, delimiter=",", header="it,kld_phi,kld_psi,kld_ram", comments="")
    ...
    if plot_dir is not None:
        ...
        hists_test_ = np.concatenate(
            [hists_test[:, : (3 * ncarts - 6)], np.zeros((nbins, 6)), hists_test[:, (3 * ncarts - 6) :]], axis=1
        )
        ...
        hists_gen_ = np.concatenate(
            [hists_gen[:, : (3 * ncarts - 6)], np.zeros((nbins, 6)), hists_gen[:, (3 * ncarts - 6) :]], axis=1
        )
        ...
        for i in range(len(label)):
            ...
            plt.savefig(os.path.join(plot_dir, "marginals_%s_%07i.png" % (label[i], iter + 1)), dpi=300)
            plt.close()
        ...
        plt.hist2d(phi, psi, bins=64, norm=mpl.colors.LogNorm(), range=[[-np.pi, np.pi], [-np.pi, np.pi]])
...
def filter_chirality(x, ind=[17, 26], mean_diff=-0.043, threshold=0.8):
    ...
    diff_ = torch.column_stack(
        (x[:, ind[0]] - x[:, ind[1]], x[:, ind[0]] - x[:, ind[1]] + 2 * np.pi, x[:, ind[0]] - x[:, ind[1]] - 2 * np.pi)
    )
