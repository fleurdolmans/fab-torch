...
def run(cfg: DictConfig):
    ...
    no_init = True
    if no_init:
        model_names = ["target_kld", "flow_nis", "flow_kld", "rsb", "snf", "craft", "fab_no_buffer", "fab_buffer"]
        titles = [
            "Flow w/ ML",
            r"Flow w/ $D_{\alpha=2}$",
            "Flow w/ KLD",
        ...
        model_names = [None, "target_kld", "flow_kld", "rsb", "snf", "craft", "fab_no_buffer", "fab_buffer"]
        titles = [
            "Initialisation",
            "Flow w/ ML",
            "Flow w/ KLD",
...
def plot_alpha_study(cfg: DictConfig):
    ...
    titles = [r"$\alpha=0.25$", r"$\alpha=0.5$", r"$\alpha=1$", r"$\alpha=1.5$", r"$\alpha=2$", r"$\alpha=3$"]
    target = setup_target(cfg, num_samples=1)
