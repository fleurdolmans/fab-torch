...
python ${LOGS_DIR}/${PROJECT_NAME}/experiments/solvation/run_LJ.py \\
  ...
  flow.hidden_units=128 flow.base.type=autoregressive-shell flow.base.learn_mean_var=false flow.type=lj_torus_spline_noF\\
  flow.n_layers=6 flow.layer_nodes_per_dim=4 flow.hidden_units=128 flow.num_bins=8\\
  training.lr=5e-5 training.wd=1e-6 training.batch_size=64 evaluation.eval_batch_size=32\\
  ...
  training.n_iterations=400 training.buffer.use=false training.buffer.prioritised=false training.lr_scheduler.decay_iter=400\\
  evaluation.n_eval=8 evaluation.n_plots=8 evaluation.n_checkpoints=1 
EOF
