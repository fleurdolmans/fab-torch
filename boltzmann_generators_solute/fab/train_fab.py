...
from target       import SoluteFullTarget2D
...
def build_flow(system):
    """Build or load the RealNVP flow for all 37 particles (74D)."""
    bg = BoltzmannGenerator({
        'n_blocks':    N_BLOCKS,
        'dimension':   N_PARTICLES * 2,   # 74
        'reshape':     (N_PARTICLES, 2),
        'n_nodes':     N_NODES,
        'n_layers':    N_LAYERS,
...
def build_fab_model(flow_adapter: RealNVPFlowFAB, target: SoluteFullTarget2D) -> FABModel:
...
def load_md_data(device):
    ...
    # If saved as (n_frames, 37, 2), flatten to (n_frames, 74)
    if x_md.ndim == 3:
        x_md = x_md.reshape(len(x_md), -1)
