...
from boltzmann_generators_3d.fab.sampling_methods.base import Point, get_intermediate_log_prob, get_grad_intermediate_log_prob, create_point
...
class TransitionOperator(torch.nn.Module):
    ...
    def create_new_point(self, x: torch.Tensor) -> Point:
        ...
        for further details."""
        return create_point(x, self.base_log_prob, self.target_log_prob, with_grad=self.uses_grad_info)
    ...
    def grad_intermediate_target_log_prob(self, point: Point, beta: float) -> torch.Tensor:
        with torch.no_grad():
            ...
            return get_grad_intermediate_log_prob(point, beta, alpha=self.alpha, p_target=self.p_target)
