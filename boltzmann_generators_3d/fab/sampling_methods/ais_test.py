...
from boltzmann_generators_3d.fab.sampling_methods import AnnealedImportanceSampler, Metropolis, HamiltonianMonteCarlo
from boltzmann_generators_3d.fab.utils.logging import ListLogger
from boltzmann_generators_3d.fab.target_distributions import TargetDistribution
...
from boltzmann_generators_3d.fab.sampling_methods.base import resample
...
def test_ais_num_dist(dim: int = 1, batch_size: int = 10000) -> None:
    ...
    base_dist = WrappedTorchDist(
        torch.distributions.MultivariateNormal(loc=torch.zeros(dim)+0.5, scale_tril=torch.eye(dim))
    )
    ...
    for n_ais_intermediate_distributions in n_intermedaite_distributions_list:
        ...
        info = {}
        info.update(
            log_Z_estimate=log_Z_estimate, ess=ess, n_ais_intermediate_distributions=n_ais_intermediate_distributions
        )
        logger.write(info)
...
def test_ais__eval_batch(
    ...
    base_samples, base_log_w, ais_samples, ais_log_w = ais.generate_eval_data(outer_batch_size, inner_batch_size)
