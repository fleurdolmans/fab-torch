...
class BoltzmannGenerator:
    def __init__(self, model_params=None):
        ...
        defaults = {'n_blocks': 3, 'dimension': 2, 'n_nodes': 100, 'n_layers': 3,
                    ...
                    'patience': 50, 'min_delta': 1e-4, 'w_overlap': 0.0}
        ...
        for i in tqdm(range(self.n_epochs)):
            ...
            for batch_x, batch_z in zip(subdata_x, subdata_z):  # iterations
                ...
                if self.w_loss[1] != 0:
                    ...
                    if getattr(self, 'w_overlap', 0.0) > 0.0:
                        from generator import overlap_penalty
                        x_kl, _ = model.generator(batch_z.detach())
                        loss_KL = loss_KL + self.w_overlap * overlap_penalty(
                            x_kl, model.sys_dim, model.system.sigma)
                if self.w_loss[2] != 0:
            ...
            if epochs_no_improve >= self.patience:
                print(f'\nEarly stopping at epoch {i + 1} (no improvement for {self.patience} epochs)')
                break
    ...
    def save(self, model, save_path, previous_loss=None):
        ...
        if 's_net' in model_params:
            del model_params['s_net']
        if 't_net' in model_params:
            del model_params['t_net']
        if 'mask' in model_params:
            del model_params['mask']
        if 'params' in model_params:
            del model_params['params']
        if 'prior' in model_params:
            del model_params['prior']
        if 'loss_iteration' in model_params:
            del model_params['loss_iteration']
        ...
        for i in tqdm(range(self.n_epochs)):
            ...
            for batch_x, batch_z in zip(subdata_x, subdata_z):  # iterations
                ...
                if w_loss[1] != 0:
                    ...
                    if getattr(self, 'w_overlap', 0.0) > 0.0:
                        from generator import overlap_penalty
                        x_kl, _ = model.generator(batch_z.to(device).detach())
                        loss_KL = loss_KL + self.w_overlap * overlap_penalty(
                            x_kl, model.sys_dim, model.system.sigma)
                if w_loss[2] != 0:
